...
interface FooterLink {
  label: string;
  to: string;
  disabled?: boolean; // 演示站点暂无对应页面，渲染为不可点文本
}
...
const footerLinks: { title: string; links: FooterLink[] }[] = [
  {
    title: "产品",
    links: [
      { label: "特性", to: "/", disabled: true },
      { label: "探索", to: "/explore" },
      { label: "仪表盘", to: "/dashboard" },
      { label: "自托管指南", to: "/", disabled: true },
    ],
  },
  {
    title: "资源",
    links: [
      { label: "文档", to: "/", disabled: true },
      { label: "API 参考", to: "/", disabled: true },
      { label: "更新日志", to: "/", disabled: true },
      { label: "状态", to: "/", disabled: true },
    ],
  },
  {
    title: "社区",
    links: [
      { label: "GitHub", to: "/", disabled: true },
      { label: "讨论区", to: "/", disabled: true },
      { label: "贡献指南", to: "/", disabled: true },
      { label: "行为准则", to: "/", disabled: true },
    ],
  },
];
    ...
    <footer className="relative mt-32 border-t border-line">
      <div className="mx-auto grid max-w-[1400px] gap-12 px-6 py-16 md:grid-cols-[1.5fr_1fr_1fr_1fr] md:px-8">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 pt-2">
            <span
              className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-line-subtle text-ink-mute"
              title="即将推出"
            >
          ...
          <div key={group.title} className="flex flex-col gap-3">
            ...
            <ul className="flex flex-col gap-2.5">
              {group.links.map((link) => (
                <li key={link.label}>
                  {link.disabled ? (
                    <span className="cursor-default text-sm text-ink-mute">{link.label}</span>
                  ) : (
                    <Link
                      to={link.to}
      ...
      <div className="border-t border-line-subtle">
        <div className="mx-auto flex max-w-[1400px] flex-col items-center justify-between gap-3 px-6 py-6 text-xs text-ink-mute md:flex-row md:px-8">
          <p className="font-mono">© {new Date().getFullYear()} Nebula OpenSource · MIT License</p>
